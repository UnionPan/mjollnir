...
class SyntheticOptionChainGenerator:
    ...
    def __init__(
        self,
        regime_vol_profiles: dict[int, RegimeVolatilityProfile] | None = None,
        ...
        if maturities_days is None:
            # Crypto-standard ladder: weekly expiries + monthlies
            maturities_days = [7, 14, 21, 28, 56, 84]
        if moneyness_range is None:
            # Liquidity concentrates at round percentages
            moneyness_range = [0.85, 0.90, 0.95, 0.975, 1.0, 1.025, 1.05, 1.10, 1.15, 1.20]
...
def quick_generate_option_chains(
    ...
    spot_symbol: str = 'BTC',
    atm_iv_per_regime: dict[int, float] | None = None,
